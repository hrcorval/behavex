...
BEHAVE_ARGS = [
    'no_color', 'color', 'define', 'exclude', 'include', 'no_junit', 'junit',
    'junit_directory', 'steps_catalog', 'no_skipped', 'show_skipped', 'lang',
    'no_snippets', 'snippets', 'no_multiline', 'multiline', 'no_capture',
    'name', 'capture', 'no_capture_stderr', 'capture_stderr', 'no_logcapture',
    'logcapture', 'logging_level', 'logging_format', 'logging_datefmt',
    'logging_filter', 'logging_clear_handlers', 'no_summary', 'summary',
    'outfile', 'quiet', 'no_source', 'show_source', 'stage', 'stop', 'tags',
    'no_timings', 'show_timings', 'verbose', 'wip', 'expand', 'lang_list',
    'lang_help', 'tags_help'
]

BEHAVEX_ARGS = [
    'output_folder', 'config', 'dry_run', 'tags', 'parallel_element', 'parallel_processes'
]
...
def parse_arguments(args):
    ...
    parser.add_argument('--exclude',
                        help="Don't run feature files matching regular ex"
                             "pression PATTERN.",
                        required=False)
    parser.add_argument('-i',
                        '--include',
                        help="Only run feature files matching regular "
                             "expression PATTERN.",
                        required=False)
    ...
    parser.add_argument('--no-capture',
                        '--no_capture',
                        help="Don't capture stdout (any stdout output will"
    ...
    parser.add_argument('--capture',
                        ...
                        action="store_true",
                        required=False)
    parser.add_argument('--no-capture-stderr',
                        '--no_capture_stderr',
                        help="Don't capture stderr (any stderr output wil"
                             "l be printed immediately.)",
                        default=False,
                        action="store_true",
                        required=False)
    parser.add_argument('--capture-stderr',
                        ...
                        action="store_true",
                        required=False)
    parser.add_argument('--no-logcapture',
                        '--no_logcapture',
                        help="Don't capture logging. Logging configuration"
                             "will be left intact.",
                        action="store_true",
                        default=False,
                        required=False)
    parser.add_argument('--log-capture',
                        '--log_capture',
    ...
    parser.add_argument('--parallel-processes',
                        ...
                        required=False)
    parser.add_argument('--parallel-element',
