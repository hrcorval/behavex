...
from behavex.reports.report_utils import create_log_path, RETRY_SCENARIOS
...
def before_scenario(context, scenario):
    ...
    try:
        ...
        if retry_scenario:
            ...
            message = '{1}{0}{1} Retrying Test Scenario... {1}{0}'.format('*' * 34, '\n' * 2)
            _log_exception_and_continue(message, '')
        log_filename = os.path.join(context.log_path, "scenario.log")
        file_handler = logging.FileHandler(log_filename, mode, encoding=LOGGING_CFG['file_handler']['encoding'])
