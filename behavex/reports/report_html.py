...
from behavex.reports.report_utils import try_operate_descriptor
...
def _create_manifest(relative, page):
    ...
    output_text = T_HANDLER.render_template(
        MANIFEST_TEMPLATE, parameters_template)
    folder = os.path.join(get_env('OUTPUT'),
                          'reports', 'bootstrap', 'manifest')
    ...
    for name_file, content in content_to_file.items():
        if name_file == 'report.html':
            ...
            layout_min_path = os.path.join(get_env('OUTPUT'),
                                           'reports', 'bootstrap', 'css', 'layout.min.css')
            ...
            path_file = os.path.join(
                get_env('OUTPUT'), 'reports', name_file)
            _create_manifest('', name_file)
        try:
...
def export_metrics_to_html(
    ...
    scenario_auto = [scenario for scenario in scenarios if not(
        'MANUAL' in scenario['tags'] or 'WIP' in scenario['tags'])]

    to_be_fixed = [scenario for scenario in scenarios
                   if any(i in ["TEST_TO_FIX", "BUG_TO_FIX"] for i in
                          scenario['tags']) and scenario['status'] == 'failed']
    parameters_template = {
        'skipped': skipped,
        'passed': passed,
        'failed': failed,
...
# steps_summary.keys() has been forced to be list type
    ...
    output_text = T_HANDLER.render_template(STEP_TEMPLATE_DEFINITION, parameter_template)
...
def get_value_filters(features):
    ...
    tags = {tag for feature in features for scenario in feature['scenarios']
...
def export_to_html_table_summary(features):
    ...
    for feature in features:
        ...
        for scenario in feature['scenarios']:
            fields['Total'] += 1
            if any(i in ["TEST_TO_FIX", "BUG_TO_FIX"]
        ...
        fields_total.update({field: fields_total[field] + fields[field]
                             for field in list_fields[1:6]})
    ...
    if len(features) > 0:
        fields_total['Execution Status'] = "%.2f" % (
            (float(fields_total['Executed']) /
             fields_total['Total']) * 100)

        fields_total['Pass Rate'] = "%.2f" % (
            (float(fields_total['Passed']) /
             fields_total['Total']) * 100)
