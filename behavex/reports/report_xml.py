...
from behavex.reports.report_utils import match_for_execution, get_save_function
...
from behavex.reports.report_utils import text
...
def _export_feature_to_xml(feature, isobject=True):
    ...
    def get_scenarios(feature_):
        ...
        :return:
        """
        def flatter_scenarios(scenarios): return sum([[scenario] if scenario.type == 'scenario' else scenario._scenarios
                                                      for scenario in scenarios], [])
        return flatter_scenarios(feature_.scenarios) if isobject else feature_['scenarios']
    ...
    scenarios = [scenario for scenario in get_scenarios(feature)
    ...
    failures = [scenario for scenario in scenarios
                if get_status(scenario) == 'failed' or
                get_status(scenario) == 'untested']

    bug_to_fix = [scenario for scenario in scenarios
                  if any(i in ["TEST_TO_FIX", "BUG_TO_FIX"] for i in get_tags(scenario))]

    bug_to_fix_failed = [scenario for scenario in bug_to_fix if get_status(scenario) == 'failed']

    summary = {
        'time': sum([scenario.duration if isobject else scenario['duration']
                     for scenario in scenarios]),
        'tests': len(scenarios),
    ...
    try_operate_descriptor(path_output + '.xml', get_save_function(path_output, output_text))
